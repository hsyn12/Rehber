...
public abstract class CallSummary extends ContactDetailsHistory {
	...
	private void setupSummaryViews(@NonNull final History callHistory) {
		...
		if (historyUpdated) {
			...
			int incomingSize = callHistory.getIncomingCalls().size();
			int outgoingSize = callHistory.getOutgoingCalls().size();
			int missedSize   = callHistory.getMissedCalls().size();
			int rejectedSize = callHistory.getRejectedCalls().size();
			...
			incomingRow.setOnClickListener(v -> showCalls(callHistory.filter(Type.INCOMING, Type.INCOMING_WIFI)));
			outgoingRow.setOnClickListener(v -> showCalls(callHistory.filter(Type.OUTGOING, Type.OUTGOING_WIFI)));
			missedRow.setOnClickListener(v -> showCalls(callHistory.filter(Type.MISSED)));
			rejectedRow.setOnClickListener(v -> showCalls(callHistory.filter(Type.REJECTED)));
