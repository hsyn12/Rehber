...
public class DefaultContactCommentator implements ContactCommentator {
    ...
    protected final DBContact           database;
    ...
    public DefaultContactCommentator(ContactCommentStore commentStore) {
        ...
        this.database = ((ContactStory) Over.Contacts.getContactManager()).getDatabase();
     ...
     * @return {@link Dates}
     */
    protected Dates getContactDates() {

        if (contact.getDates() != null) return contact.getDates();

        var dates = database.getDates(contact.getContactId());
    ...
    private List<Call> getContactHistory() {
        ...
        if (calls != null)
            return calls
                    .stream()
                    .filter(call -> PhoneNumbers.containsNumber(contact.getNumbers(), call.getNumber()))
    ...
    public CharSequence commentHistory() {
        ...
        if (dates != null) {
            ...
            long distance = first.getTime() - contact.getSavedDate();
    ...
    private CharSequence onInstallDate() {
        ...
        long          savedDate     = contact.getSavedDate();
        ...
        List<Long> allDates = Lister.mapNotNull(contacts, Contact::getDates)
                .stream()
                .map(Dates::getSavedDate)
                .collect(Collectors.toList());
